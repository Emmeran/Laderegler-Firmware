...
#define VGEN_CALIBRATION 5 * 443 / 1023
...
void messeBatterieSpannung()
{
	...
	vbatRingBuffer[vbatRingBufferPos] = analogRead(VSENSE_BAT);
...
void messeGeneratorSpannung()
{
    vgen = analogRead(VSENSE_GEN) * VGEN_CALIBRATION;
}

// prüfe Batterie Spannung
void pruefeBatterieSpannung()
{
    messeGeneratorSpannung();
	...
	schalteGeneratoren( AUS );
    }
    
    else if (deviceState == STATE_OVERPOWER && BATTERIE_NORMAL)
    {
	deviceState = STATE_NORMAL;
	lastStateChange = millis();
	
	schalteGeneratoren( AN );
    }
